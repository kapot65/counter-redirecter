...
import utils.rsb_event_pb2 as rsb_event_pb2
...
if __name__=='__main__':
            ...
            for j in range(n_blocks):
                ...
                if data is None:
                    for k, time in enumerate(times):
                        block.peaks.times.append(time)
                        if (not amps is None):
                            block.peaks.amplitudes.append(amps[k])
                else:
                    for k, time in enumerate(times):
    ...
    test_writing(np.random.randint(0, 2**32, n_samples), data_size=0)
    
    
    print('test #2: point with amp events only, amp < 16 bit')
    test_writing(np.random.randint(0, 2**32, n_samples),
                 amps=np.random.randint(0, 2**15, n_samples, np.uint16), 
                 data_size=2)
    
    print('test #3: point with amp events only, amp > 16 bit')
    test_writing(np.random.randint(0, 2**32, n_samples),
                 amps=np.random.randint(0, 2**32, n_samples), data_size=4)
    
    print('test #4: point with data only')
    test_writing(np.random.randint(0, 2**32, n_samples),
                 data=np.random.randint(0, 2**32, (n_samples, 128)), 
                 data_size=128*2)
